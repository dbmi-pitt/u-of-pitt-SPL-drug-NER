...
public class ExtractAbbrev {
    ...
    protected List<String> extractAbbrPairs(String inFile) {
		...
		do {
			...
			if (shortForm.length() > 1 && longForm.length() > 1) {
			    ...
			    if (isValidShortForm(shortForm)){
			    	abbrevs =  extractAbbrPair(shortForm.trim(), longForm.trim());
			    	if (abbrevs != null) {
			    		candidates.add(abbrevs);
			    	}
			    }
			}
...
//	    System.out.println(shortForm + delimiter + bestLongForm);
	    return shortForm + delimiter + bestLongForm;
	}
	return null;
    }
